    ...
    join(cl) { //this stuff is complicated
        ...
        if (!otheruser) {
            ...
            if (((this.connections.length == 0 && Array.from(this.ppl.values()).length == 0) && !this.isLobby(this._id)) || this.crown && (this.crown.userId == cl.user._id)) { //user that created the room, give them the crown.
                ...
                this.crowndropped = false;
                this.settings = {visible:this.settings.visible ? this.settings.visible : true, color:this.settings.color ? this.settings.color : this.server.defaultRoomColor, color2:this.settings.color2 ? this.settings.color2 : this.server.defaultRoomColor2, chat:this.settings.chat ? this.settings.chat : true, crownsolo:this.settings.crownsolo ? this.settings.crownsolo : false};
            } else {
                //cl.quotas.a.setParams(Quota.PARAMS_A_NORMAL);
                this.settings = {visible:this.settings.visible ? this.settings.visible : true, color:this.settings.color ? this.settings.color : this.server.defaultRoomColor, color2:this.settings.color2 ? this.settings.color2 : this.server.defaultRoomColor2, chat:this.settings.chat ? this.settings.chat: true, crownsolo:this.settings.crownsolo ? this.settings.crownsolo : false, lobby:this.settings.lobby ? this.settings.lobby : true};
            }
